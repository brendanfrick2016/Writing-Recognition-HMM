...
class HMM:
    ...
    def label( self, data ):
        ...
        for state in self.states:
            delta[state] = self.priors[state] * self.getEmissionProb(state,data[0])
        ...
        for index in range(1, n):
            ...
            for state_to in self.states:
                max_prob = 0
                max_state = None
                # Find which state is the most probable 
                for state_from in self.states:
                    prob = delta[state_from] * self.transitions[state_from][state_to] #Define probability of moving from current state to next state
                    #print 'state ', state_from,' prob ', prob
                    if prob > max_prob:
                        max_prob = prob
                        max_state = state_from
                delta_bar[state_to] = max_prob * self.getEmissionProb(state_to,data[index]) # Save probability of state to key state (factoring in feature scores)
                pre_state[state_to] = max_state #Save most probable pre-state
            delta = delta_bar # Set next state
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length','curvature','velocity']
        self.contOrDisc = {'length': DISCRETE, 'curvature':CONTINUOUS,'velocity': DISCRETE}
        self.numFVals = { 'length': 2, 'curvature':2, 'velocity':2}
    ...
    def labelFile( self, strokeFile, outFile ):
        ...
        self.confusion(trueLabels, labels)
    ...
    def confusion(self,trueLabels,classifications):
        ret = {'drawing' : {'drawing': 0, 'text' : 0}, 'text' : {'drawing' : 0, 'text' : 0}}
        for i in range(len(trueLabels)):
            ret[trueLabels[i]][classifications[i]] += 1
        print "\nCONFUSION MATRIX:\n"
        print "True Label\t Classified as Drawing\t Classified as Text\t Percent Correct"
        print "Drawing:  \t\t",  ret['drawing']['drawing'],'\t\t\t ', ret['drawing']['text'],'\t\t\t', float(ret['drawing']['drawing'])/float(ret['drawing']['drawing']+ret['drawing']['text'])
        print "Text:  \t\t\t",  ret['text']['drawing'],'\t\t\t ', ret['text']['text'],'\t\t\t', float(ret['text']['text'])/float(ret['text']['drawing']+ret['text']['text'])
        print "\n"
        return ret
...
class Stroke:
    ...
    def velocity(self):
        ...
        for n in range(len(self.points)-1):
            count += 1
            # vel = d/t
            xdiff = self.points[n][0] - self.points[n+1][0]
            ydiff = self.points[n][1] - self.points[n+1][1]
            ret += math.sqrt(xdiff**2 + ydiff**2)   
            vel += float(ret)/len(self.points)
        return vel
